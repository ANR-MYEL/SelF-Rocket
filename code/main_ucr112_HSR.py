...
if __name__ == '__main__':
    ...
    for i in range(len(datasets)):
        ...
        for j in tqdm(range(num_resamples)):
            ...
            model = HydraSelFRocket(num_runs = num_runs, num_features_pc = num_features_pc, only_MIX=bool_only_mix, num_kernels = 10000)
            model.fit(X_train,y_train)
            y_hsr = model.predict(X_test)
            accuracy_HSR = accuracy_score(y_test,y_hsr)
            dataset_perf_HSR.append(accuracy_HSR)
            dataset_ir_po_HSR.append(model.selected_comb_po)
        output_perf_by_rsmpl=os.path.join(output_path,"Perf_rsmpl_HSR_UCR112_fpc{}_nr{}.csv".format(num_features_pc,num_runs))
        output_main=os.path.join(output_path,"Mean_perf_HSR_UCR112_fpc{}_nr{}.csv".format(num_features_pc,num_runs))
        output_ir_po_by_rsmpl=os.path.join(output_path,"IR_PO_rsmpl_HSR_UCR112_fpc{}_nr{}.csv".format(num_features_pc,num_runs))
